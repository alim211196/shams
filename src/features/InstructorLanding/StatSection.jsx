...
const StatSection = ({
  ...
  useEffect(() => {
    const existingCards = Object.keys(formData)
      .filter((key) => key.startsWith("instructor_section3_stat_title_"))
      .map((key) => {
  ...
  const addCard = () => {
    ...
    setFormData((prev) => ({
      ...prev,
      [`instructor_section3_stat_title_${newId}`]: "",
      [`instructor_section3_stat_count_${newId}`]: "",
    }));
  };
  ...
  const removeCard = (id) => {
    ...
    setFormData((prev) => {
      ...
      delete updatedFormData[`instructor_section3_stat_title_${id}`];
      delete updatedFormData[`instructor_section3_stat_count_${id}`];
      ...
      {cards.map((card) => (
        ...
        >
          ...
          <Grid size={{ xs: 12, md: 4 }}>
            <CustomLabel label="Stats">
              <TranslateButton />
            </CustomLabel>
          </Grid>
          <Grid size={{ xs: 12, md: 4 }}>
            <CustomTextField
              name={`instructor_section3_stat_title_${card.id}`}
              value={
                formData[`instructor_section3_stat_title_${card.id}`] || ""
              }
          ...
          <Grid size={{ xs: 12, md: 4 }}>
            <CustomTextField
              name={`instructor_section3_stat_count_${card.id}`}
              type="number"
              value={
                formData[`instructor_section3_stat_count_${card.id}`] || ""
              }
              onChange={handleChange}
